...
namespace calcaot.ViewModels
{
    public class MainViewModel : ViewModelBase
    {
        ...
        public MainViewModel(IDialogService dialogService, IThemeService themeService)
        {
            ...
            History.CommandExecuted += (_, __) =>
                DiaryCaretaker.GetInstance().ScheduleSave(CreateMemento());
        }

        public DiaryMemento CreateMemento()
        {
            var groups = MealGroups.Select(g => new MealGroupSnapshot(
                g.Name,
                g.Foods.Select(f => new FoodSnapshot(
                    f.Id,
                    f.Name,
                    f.Weight,
                    f.Calories,
            ...
            return new DiaryMemento(groups);
        }

        public void RestoreMemento(DiaryMemento memento)
        {
            if (memento == null) return;

            foreach (var groupSnap in memento.MealGroups)
            {
                var vm = MealGroups.FirstOrDefault(g => g.Name == groupSnap.Name);
                if (vm is null)
                    continue;

                foreach (var f in groupSnap.Foods)
                {
                    vm.AddFood(new FoodItem
                    {
                        Id = f.Id,
                        Name = f.Name,
                        Weight = f.Weight,
